...
SET "logfile=P4_logfile_%YYYYMMDD%_%logtime%"
...
for %%a in ("%cd%\temp\*.mkv*") do (
	...
	echo %%~na ; %date:~6,4%-%date:~3,2%-%date:~0,2% >> "%cd%\logs\db\P4_database.csv
	)
...
echo %date:~6,4%-%date:~3,2%-%date:~0,2% !TIME:~0,8! Copying all .mp4 files to /MP4. >> "%cd%\P4\logs\%logfile%.log"
copy P4\temp\*.mp4 "%cd%\MP4"
echo %date:~6,4%-%date:~3,2%-%date:~0,2% !TIME:~0,8! Finished copy. >> "%cd%\P4\logs\%logfile%.log"


echo %date:~6,4%-%date:~3,2%-%date:~0,2% !TIME:~0,8! Sorting all files into folders. >> "%cd%\P4\logs\%logfile%.log"
cd %cd%\P4\temp
...
timeout 5
echo %date:~6,4%-%date:~3,2%-%date:~0,2% !TIME:~0,8! Making checksums for all sorted files. >> "%cd%\P4\logs\%logfile%.log"
:: Lager chcksum av mapper inne i temp
fsum "%cd%\P4\temp" /T:F /R /O
echo %date:~6,4%-%date:~3,2%-%date:~0,2% !TIME:~0,8! Checksums done. >> "%cd%\P4\logs\%logfile%.log"

timeout 5
echo %date:~6,4%-%date:~3,2%-%date:~0,2% !TIME:~0,8! Moving all sorted files to /Ferdig. >> "%cd%\P4\logs\%logfile%.log"
:: Flytter mappene til /ferdig - inneholder mkv, xml, xml, txt, md5. /XF er redundant atm.
robocopy "%cd%\P4\temp" "%cd%\Ferdig" /MOVE /XF "mediaconch.exe" "libcurl.dll" /E
timeout 5
...
timeout 5
echo %date:~6,4%-%date:~3,2%-%date:~0,2% !TIME:~0,8! Moving done. >> "%cd%\P4\logs\%logfile%.log"
echo %date:~6,4%-%date:~3,2%-%date:~0,2% !TIME:~0,8! End logfile. >> "%cd%\P4\logs\%logfile%.log"
...
echo ------------------------------------ >> "%cd%\P4\logs\%logfile%.log"
echo START:       %STARTTIME:~0,8%     %STARTDATE% >> "%cd%\P4\logs\%logfile%.log"
echo END:         %ENDTIME:~0,8%     %ENDDATE% >> "%cd%\P4\logs\%logfile%.log"
echo ------------------------------------ >> "%cd%\P4\logs\%logfile%.log"
...
echo DURATION:    %DURATIONH%:%DURATIONM%:%DURATIONS% >> "%cd%\P4\logs\%logfile%.log"
echo ------------------------------------ >> "%cd%\P4\logs\%logfile%.log"
